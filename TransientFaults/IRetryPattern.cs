...
namespace TransientFaults {
    public interface IRetryPattern {
        ...
        Task<T> RetryAsync<T>(Func<CancellationToken,Task<T>> funcAsync,CancellationToken token,ICircuitBreaker circuitBreaker,Func<Task<T>> circutOpenValue,RetryPattern.Config config = null,Func<T,bool> retryIfTrue = null);
